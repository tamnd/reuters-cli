// Package cli builds the reuters command tree on top of the reuters library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: reuters.DefaultConfig()}

	root := &cobra.Command{
		Use:   "reuters",
		Short: "Read Reuters world news via RSS",
		Long: `reuters reads news from Reuters via its public RSS feeds.
No API key is required. It returns records as table, JSON, JSONL,
CSV, TSV, or URLs.

reuters is an independent tool and is not affiliated with Reuters.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = reuters.NewClient(a.cfg)
	return nil
}
