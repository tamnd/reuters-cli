...
describes what users get. Pick a format with `-o`, or let reuters choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
reuters <command> -o table   # aligned columns for reading
reuters <command> -o jsonl   # one JSON object per line, for piping
reuters <command> -o json    # a single JSON array
reuters <command> -o csv     # spreadsheet friendly
reuters <command> -o tsv     # tab-separated
reuters <command> -o url     # just the URL column
reuters <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
reuters <command> --fields id,title,url
```
...
```bash
reuters <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
reuters <command>            # a table, because this is a terminal
reuters <command> | wc -l    # JSONL, because this is a pipe
```
